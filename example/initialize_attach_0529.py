...
class Test():
    def __init__(self):
        ...
        self._asset_path = self._path + "/example/asset" ### BONG
        self.max_episode_length = 100 ### BONG
    ...
    def get_rigid_pole(self):
        ...
        init_positions[:, 1] = random.uniform(1.5, 1.8)
    ...
    def get_random_gripper_pose(self, sliced_positions):
        ...
        init_positions[:, 2] = _init_point[2] + 0.16
        
        # init_positions[:, 2] = 0.7
        init_orientation = [0,0,0]
        # init_orientation[0] = random.randint(-30, 30)
        # init_orientation[1] = random.randint(-30, 30)
    ...
    def get_bounding_points(self, r, random_pose, rand=False):
        ...
        if rand == False:
            ...
            # check random pose
            # aa, _ = self.gripper.get_world_poses()
            # aa *= 0 
            # self.gripper.set_world_poses(positions=aa)
            self.world.step()
            self.world.pause()
            random_pose[0][:,2] -= 0.01
            ...
            local_collision_point = (np.array(rigid_body.GetPointsAttr().Get()))
            vertices = np.array(local_collision_point)
            vertices_tf_row_major = np.pad(vertices, ((0, 0), (0, 1)), constant_values=1.0) #(313, 4)
            ##################
            
            # Row-major transformation matrix
            source_to_world_row_major_tf = UsdGeom.Xformable(get_prim_at_path('/World')).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
            # target_to_world_row_major_tf = UsdGeom.Xformable(target_prim).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
            # from IPython import embed; embed(); exit()

            source_to_world_column_major_tf = np.transpose(T_1)
            target_to_world_column_major_tf = np.transpose(source_to_world_row_major_tf)
            world_to_target_column_major_tf = np.linalg.inv(target_to_world_column_major_tf)            
            relative_tf_row_major_2 = world_to_target_column_major_tf @ source_to_world_column_major_tf
            
            source_to_world_row_major_tf = UsdGeom.Xformable(get_prim_at_path(r)).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
            target_to_world_row_major_tf = UsdGeom.Xformable(get_prim_at_path('/World')).ComputeLocalToWorldTransform(Usd.TimeCode.Default())
            ...
            relative_tf_row_major = np.transpose(source_to_target_column_major_tf)
            # from IPython import embed; embed(); exit()

            # Transform points so they are in the coordinate system of the top-level ancestral xform prim
            points_in_relative_coord = vertices_tf_row_major @ relative_tf_row_major @ relative_tf_row_major_2
                                        # 313, 4 / 4 * 4
                                       #franka_leftfingerpcd > franka left finger> world > remove translation  > random 
            points_in_meters = points_in_relative_coord[:, :-1]
    ...
    def main(self):
        ...
        # self.pcd = PointCloudHandle(deform_path=self.deform_path)
        ...
        while simulation_app.is_running():
            ...
            if i == self.max_episode_length: # randomize reset (max_episode_length)
                ...
                while TOUCH == False:
                    ...
                    # generate and check free-collision pose
                    if tip1 == False and tip2 == False and tr == True:
                        TOUCH = True
                        # print("checek")
                        self.random_pose[0][:,2] += -1
                ...
                if TOUCH:
                    world.step()
                    world.play()
                    self.gripper.set_world_poses(positions=self.random_pose[0], orientations=self.random_pose[1])
                    # print("move")
                    world.pause()
                    self.set_attach(rigid=self.attach_prim, deform=self.deform_path)
                    self.set_attach(rigid=self.attach_prim, deform=self.obstacle_prims[1])
                    self.set_attach(rigid=self.attach_prim, deform=self.obstacle_prims[2])
